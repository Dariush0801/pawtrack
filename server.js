...
const server = http.createServer((req, res) => {
  ...
  // API: Send Google Account Authorization & Scope Security Notification to Gmail
  if (parsedUrl === '/api/send-auth-notification' && req.method === 'POST') {
    let body = '';
    req.on('data', chunk => { body += chunk; });
    req.on('end', () => {
      try {
        ...
        const recipientName = (payload.name || 'Pet Guardian').trim();
        const timestamp = payload.authTime || new Date().toISOString();

        if (!targetEmail || !targetEmail.includes('@')) {
          res.writeHead(400, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({ error: 'Valid Google/Gmail address required' }));
          return;
        }
        ...
        console.log('\n---------------------------------------------------------');
        console.log(`[PawTrack Auth Notification] Real-Time Notice Dispatched To: ${targetEmail}`);
        console.log(`[PawTrack Auth Notification] Recipient Name: ${recipientName}`);
        console.log(`[PawTrack Auth Notification] Subject: "Security Alert: Google Account Connected to PawTrack"`);
        console.log(`[PawTrack Auth Notification] Scopes: Profile | Guardian Credentials | Municipal Impound Alerts`);
        console.log(`[PawTrack Auth Notification] Category: Google / Primary Inbox | SMTP: ${emailSentViaSmtp}`);
        console.log('---------------------------------------------------------\n');
        ...
        res.end(JSON.stringify({
          ...
          message: `Google Account authorization security notification dispatched to ${targetEmail}.`,
          timestamp: new Date().toISOString()
        }));
      } catch (err) {
        res.writeHead(400, { 'Content-Type': 'application/json' });
